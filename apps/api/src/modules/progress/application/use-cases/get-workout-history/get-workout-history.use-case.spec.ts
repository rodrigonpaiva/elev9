...
describe("GetWorkoutHistoryUseCase", () => {
  ...
  it("returns ordered workout history successfully", async () => {
    ...
    expect(workoutLogRepository.findByTrainingPlanIdsOrdered).toHaveBeenCalledWith({
  ...
  it("returns empty array when no active training plan exists", async () => {
    ...
    expect(workoutLogRepository.findByTrainingPlanIdsOrdered).not.toHaveBeenCalled();
  });
