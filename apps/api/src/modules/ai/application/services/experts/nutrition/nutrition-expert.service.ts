...
export class NutritionExpert extends BaseCoachExpert {
  constructor() {
    super({
      ...
      category: 'NUTRITION',
      supportedIntents: ['NUTRITION', 'GOALS', 'RECOVERY', 'PLANNING', 'MOTIVATION'],
      supportedDomains: ['nutrition', 'goals', 'recovery', 'training'],
      estimatedCost: 2,
      estimatedLatencyMs: 18,
      priority: 90,
      capabilities: ['NUTRITION_SPECIALIST', 'COACH_ROUTING', 'CONTEXT_SYNTHESIS'],
      enabled: true,
    });
  }

  override loadContext(input: CoachExpertRequest, context: CoachExpertContext): CoachExpertContext {
    void input;
  ...
  override analyze(input: CoachExpertRequest, context: CoachExpertContext): CoachExpertResult {
  ...
  override contribute(_input: CoachExpertRequest, _context: CoachExpertContext, result: CoachExpertResult): readonly CoachExpertContribution[] {
    return result.contributions;
  }

  private buildAnalysis(context: CoachExpertContext, userMessage = ''): NutritionAnalysis {
    const nutrition = context.nutritionContext ?? context.healthContext?.nutritionContext ?? unavailableCoachNutritionContext();
    ...
    return Object.freeze({
      ...
      priority,
      signals: Object.freeze(['source=nutrition_read_model', `availability=${nutrition.availability}`, `freshness=${nutrition.freshness}`]),
  ...
  private buildResponse(nutrition: CoachNutritionContext, message: string): NutritionCanonicalResponse {
    const facts: NutritionExplainabilityFact[] = ['availability', 'freshness'];
    const action = nutrition.focus?.action ?? nutrition.insight?.action ?? nutrition.actions[0] ?? { type: 'none' as const };
    const normalized = message.toLowerCase();
    let text: string;
    if (this.isSafetyQuestion(normalized)) {
      text = 'I can explain your current nutrition targets and logged progress, but I cannot prescribe a diet or assess a medical condition. For personalised medical guidance, speak with a qualified healthcare professional.';
    } else if (nutrition.availability === 'not_configured') {
      text = nutrition.focus?.message ?? 'Your nutrition setup is not complete yet. Finish your nutrition profile to start receiving daily targets.';
      facts.push('focus');
      ...
      text = nutrition.focus?.message ?? nutrition.insight?.message ?? 'There is not enough information to summarize today’s nutrition progress.';
      if (nutrition.focus) facts.push('focus');
      if (nutrition.insight) facts.push('insight');
    } else if (normalized.includes('protein') || normalized.includes('proteína')) {
    ...
    } else if (normalized.includes('calorie') || normalized.includes('caloria')) {
      facts.push('calorie_progress');
      text = this.calorieText(nutrition);
    } else if (normalized.includes('focus') || normalized.includes('agora') || normalized.includes('foco')) {
      facts.push('focus');
      text = nutrition.focus?.message ?? 'There is no current nutrition focus available.';
    } else {
    ...
    if (nutrition.freshness === 'stale') text += ' This nutrition summary may not include your latest updates.';
    if (nutrition.freshness === 'legacy') text += ' Some nutrition information may need to be refreshed.';
    return Object.freeze({ text, factsUsed: Object.freeze([...new Set(facts)]), action });
  }

  private calorieText(nutrition: CoachNutritionContext): string {
    if (!nutrition.calories || nutrition.calories.target === null) return 'Calorie progress is available, but no calorie target is currently configured.';
    if (nutrition.calories.state === 'above_target') return nutrition.calories.excess === null ? 'Your logged intake is above today’s calorie target.' : `Your logged intake is above today’s calorie target by ${nutrition.calories.excess} calories.`;
    return nutrition.calories.remaining === null ? `You have logged ${nutrition.calories.consumed} calories toward a target of ${nutrition.calories.target}.` : `You have ${nutrition.calories.remaining} calories remaining today.`;
  }

  private isSafetyQuestion(message: string): boolean {
    return ['diet', 'allerg', 'supplement', 'medication', 'medical', 'diagnos', 'lose 10 kg', 'remédio'].some((term) => message.includes(term));
  }
  ...
  private mapStatus(nutrition: CoachNutritionContext): NutritionStatus {
    ...
    switch (nutrition.adherenceStatus) {
      case 'within_range': return 'ON_TRACK';
      case 'above_range': return 'MISSED';
      default: return 'PARTIAL';
    }
  }

  private mapPriority(nutrition: CoachNutritionContext): NutritionPriority {
    return nutrition.availability === 'processing_failed' || nutrition.availability === 'not_available' ? 'HIGH' : nutrition.availability === 'not_configured' ? 'CRITICAL' : nutrition.availability === 'insufficient_data' ? 'MEDIUM' : 'LOW';
  }

  private mapAction(type: CoachNutritionContext['actions'][number]['type']): NutritionRecommendationCode {
  ...
  private emptyMacroAssessment(): NutritionAnalysis['macroAssessment'] {
    const target = (summary: string) => ({ target: 0, consumed: 0, delta: 0, ratio: null, status: 'UNKNOWN' as const, summary });
    return { calories: target('Unavailable'), protein: target('Unavailable'), carbs: target('Unavailable'), fat: target('Unavailable'), overallStatus: 'UNKNOWN' as NutritionStatus, adherencePercentage: 0, summary: 'Canonical macro progress is presented by the read model.' };
  }

  private emptyMealAssessment(): NutritionAnalysis['mealAssessment'] {
    return { mealTiming: 'UNKNOWN', mealStatuses: [], completedCount: 0, partialCount: 0, missedCount: 0, pendingCount: 0, totalMeals: 0, nextMealId: null, summary: 'Canonical meal progress is presented by the read model.' };
  }

  private buildContribution(analysis: NutritionAnalysis, context: CoachExpertContext): NutritionExpertContribution {
    const recommendation = analysis.recommendations[0];
    return {
      expertId: this.metadata.id,
      summary: analysis.canonicalResponse?.text ?? 'Nutrition information is unavailable.',
      analysis,
      ...
      confidence: analysis.confidence,
      priority: analysis.priority,
      metadata: { selectionReason: context.selectionReason, recommendationCode: recommendation?.code },
    };
  }

  private buildContributions(contribution: NutritionExpertContribution): readonly CoachExpertContribution[] {
    return Object.freeze([
      { expertId: this.metadata.id, type: 'ANALYSIS', summary: contribution.summary, metadata: { kind: 'analysis' } },
      { expertId: this.metadata.id, type: 'CONTRIBUTION', summary: contribution.summary, metadata: { kind: 'contribution' } },
    ]);
  }
}
