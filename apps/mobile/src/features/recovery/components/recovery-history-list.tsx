...
export function RecoveryHistoryList({
  ...
  return (
    <View style={styles.section}>
      <SectionHeader title="Recovery history" subtitle="Your recent daily results." />
      {history.length === 0 ? (
        <Card>
            ...
            <Card style={styles.card}>
              <View style={styles.row}>
                <View style={styles.copy}>
                  ...
                  <Text style={styles.meta}>{freshnessLabel(item.freshness)}</Text>
                </View>
                <View style={styles.scoreBlock}>
                  <Text variant="title">{item.score}</Text>
                  <Badge label={categoryLabel(item.category)} variant="primary" />
                </View>
              </View>
            </Card>
          </Pressable>
