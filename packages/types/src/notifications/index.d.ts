export type NotificationType = 'recovery_alert' | 'workout_reminder' | 'missed_workout' | 'goal_milestone' | 'goal_achievement' | 'weekly_summary' | 'coach_nudge' | 'nutrition_reminder';
export type NotificationPriority = 'low' | 'medium' | 'high' | 'urgent';
export type NotificationStatus = 'planned' | 'sent' | 'opened' | 'dismissed' | 'completed' | 'skipped';
export type NotificationChannel = 'in_app' | 'push' | 'email';
export type NotificationFatigueLevel = 'low' | 'medium' | 'high';
export type NotificationSuppressionReason = 'daily_cap_reached' | 'same_type_cooldown' | 'high_dismissal_ratio' | 'already_engaged' | 'recent_notification';
export interface NotificationFatigueEvaluation {
    suppressed: boolean;
    fatigueLevel: NotificationFatigueLevel;
    reasons: NotificationSuppressionReason[];
}
export type EngagementEventType = 'impression' | 'opened' | 'clicked' | 'dismissed' | 'completed';
export type NotificationInfluenceCode = 'LOW_READINESS' | 'HIGH_FATIGUE' | 'MEDIUM_FATIGUE' | 'LOW_FATIGUE' | 'REST_DAY_RECOMMENDED' | 'GOAL_ACHIEVED' | 'GOAL_MILESTONE_CLOSE' | 'MISSED_WORKOUTS' | 'LOW_NUTRITION_ADHERENCE' | 'LOW_ENGAGEMENT' | 'COACH_CONSISTENCY_NUDGE';
...
export interface NotificationReplayDifference {
    field: 'type' | 'priority' | 'channel' | 'status' | 'title' | 'message' | 'actionLabel' | 'actionTarget' | 'influences' | 'formulaVersion' | 'generatedBy';
    persisted: unknown;
