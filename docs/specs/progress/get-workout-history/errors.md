# Errors — Get Workout History

## 1. Overview

Este documento define os erros possíveis do use-case `get-workout-history`.
...
### 4.4 WORKOUT_HISTORY_INVALID_INPUT

Usado quando o input do histórico é inválido.
...
### 4.5 WORKOUT_HISTORY_INTERNAL_ERROR
...
### 5.3 Ownership-safe history access

O histórico não deve vazar existência de logs, planos ou perfis de outros usuários.
...
WORKOUT_HISTORY_INVALID_INPUT -> 400 -> Invalid workout history input.
WORKOUT_HISTORY_INTERNAL_ERROR -> 500 -> An unexpected error occurred.
```
...
O sistema deve falhar de forma segura, mantendo separação entre sessão, perfis, planos e histórico de progresso.
